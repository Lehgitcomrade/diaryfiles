...
def forget_code():
    ...
    bg_image = ImageTk.PhotoImage(file='wave.jpg')
...
def hide():
    openeye.config(file= 'closed eye3.png')
    code.config(show= '*')
    eye_button.config(command= show)
def show():
    openeye.config(file='open eye2.png')
    code.config(show='')
...
background_image = Image.open('wave.jpg')
...
openeye= PhotoImage(file= 'open eye2.png')
...
Button(frame, width= 28, pady= 5 , text= 'Login',cursor= 'hand2', bg = 'deepskyblue4', fg= 'white', border=0, command= Login_user ). place(x=20, y= 220)

account= Label(frame, text= "Don't have an account?", fg= 'black', bg= 'Azure3', font=(Pfont,9, 'bold'))
account.place(x= 25, y=255)

sign_up= Button(frame, width= 6, text= 'Sign Up', border= 0, bg= 'Azure3', cursor= 'hand2', fg= 'deepskyblue4',  font=(Pfont,9,'bold'), command= signup_page)
