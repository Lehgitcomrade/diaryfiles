...
cursor.execute('CREATE TABLE IF NOT EXISTS Diarylite (id INT AUTO_INCREMENT PRIMARY KEY, Topic VARCHAR(100), created_at TIMESTAMP NOT NULL, story TEXT NOT NULL)')
...
def add_entry():
    ...
    # entry_topic_entry.delete("1.0", END)

    # Insert the entry into the database
    cursor.execute('INSERT INTO Diarylite (Topic, created_at, story) VALUES (%s, %s, %s)', (entry_topic, entry_date, entry_content))
    conn.commit()
...
def display_entries():
    ...
    cursor.execute("SELECT id, Topic, created_at FROM Diarylite")  # Select id, topic, and date from the Diary table
...
def show_story_popup(item):
    ...
    cursor.execute("SELECT story FROM Diarylite WHERE id = %s", (selected_item_id,))
...
def fetch_entry_content(entry_id, updated_content_entry):
    try:
        ...
        cursor.execute("SELECT story FROM Diarylite WHERE id = %s", (entry_id,))
...
def update_entry_in_db(entry_id, updated_content):
    try:
        ...
        cursor.execute("UPDATE Diarylite SET story = %s WHERE id = %s", (updated_content, entry_id))
        conn.commit()
...
def delete_entry():
    ...
    try:
        ...
        cursor.execute("DELETE FROM Diarylite WHERE id = %s", (entry_id,))
        conn.commit()
...
# create logo
image_path = 'pythonlogo1.png'
icon_image2 = Image.open(image_path)
image2 = ImageTk.PhotoImage(icon_image2)
diary.iconphoto(True, image2)

# Create widgets for your diary interface
entry_topic_label = Label(diary, text='Topic:', bg='Azure3',  bd=3)
entry_topic_label.place(x=50, y=50)
entry_topic_entry = Entry(diary, width=60, bg='Azure')
entry_topic_entry.place(x=100, y=50)

entry_story_label = Label(diary, text='Story:', bg='Azure3',  bd=3)
