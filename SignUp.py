...
def database_connect():
    if first_name.get() == '' or last_name.get() == '' or user.get() == '' or code.get() == '' or email.get() == '' or Rcode.get()== '':
        ...
        try:
            ...
            cursor.execute(Query)
            Query= 'CREATE TABLE IF NOT EXISTS logs (id int auto_increment primary key, Firstname TEXT not null, Lastname TEXT not null, Email TEXT not null, Username TEXT not null , Password varchar(20) not null, Login Timestamp)'
            cursor.execute(Query)
...
background_image= ImageTk.PhotoImage(file= 'wave.jpg')
...
project_window.iconphoto(True, image2)
